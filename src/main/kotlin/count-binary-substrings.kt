...
class Solution_count_binary_substrings {

    // T:O(n) S:O(1)
    // see explanation here: https://leetcode.com/problems/count-binary-substrings/solution/
    fun countBinarySubstrings(s: String): Int {
        ...
        for (i in 1 until s.length) {
            if (s[i - 1] != s[i]) {
                ret += java.lang.Math.min(prev, cur)
                prev = cur
                cur = 1
            } else {
                cur++
...
fun main(args: Array<String>) {
    // see [CountBinarySubstringsTest] for unit test
}