...
class Solution_baseball_game {

    // T:O(n) S:O(n)
    fun calPoints(ops: Array<String>): Int {
        // use a stack is faster than a list
        ...
        for (op in ops) {
            when (op) {
                ...
                "+" -> {
                    val n = stk.pop()
                    val newVal = n + stk.peek()
                    // push back
                    stk.push(n)
                    stk.push(newVal)
                }
                else -> stk.push(op.toInt())
            }
        }
        return stk.sum()
    }
...
fun main(args: Array<String>) {
    // LC OJ passed
    val s = Solution_baseball_game()
    println(s.calPoints(arrayOf("5", "2", "C", "D", "+")))
    println(s.calPoints(arrayOf("5", "-2", "4", "C", "D", "9", "+", "+")))
}